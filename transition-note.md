...
## Appendix B: 選択肢A（Azure OpenAI Service）アーキテクチャ構成図

### 現行との主な変更点

| コンポーネント | 現行 | 選択肢A |
|:---:|:---:|:---:|
| オーケストレーション | Claude Code（Skills / Markdown定義） | Python スクリプト（コードに内包） |
| AI バックエンド | Anthropic Claude API | Azure OpenAI Service（GPT-4o） |
| 文書処理・ファイル構造 | Python / ローカルFS | 変更なし |
...
graph TB
    ...
    subgraph IF ["インターフェース層"]
        CLI["CLI\npython main.py --mode classify|check|review"]
    end

    subgraph APP ["アプリケーション層（Python）"]
        direction TB
        subgraph ORCH ["オーケストレーション　※旧: Claude Code Skills"]
            ClassifyFlow["分類フロー\nclassify_flow()"]
            CheckFlow["チェックフロー\ncheck_flow()"]
            ReviewFlow["レビューフロー\nreview_flow()"]
        end
        subgraph COMMON ["共通コンポーネント"]
            ...
            PromptLoader["プロンプト・テンプレート管理\n分類マスター・チェックリスト読込"]
            AOAIClient["Azure OpenAI クライアント\nopenai.AzureOpenAI"]
        end
    end

    subgraph AZURE ["AI 層（Azure）"]
        AOAI["Azure OpenAI Service\nGPT-4o\n─────────────────────\nAzureテナント内処理\nデータ主権・コンプライアンス対応"]
    end

    subgraph STORAGE ["ストレージ層（ローカルファイルシステム）　※変更なし"]
        direction LR
        Input[("input/\n.docx / .doc")]
        Mid[("mid/\ncontract_*.md")]
        Direction[("direction/\nテンプレート\n分類マスター")]
    ...
    User --> CLI
    CLI -->|"--mode classify"| ClassifyFlow
    CLI -->|"--mode check"| CheckFlow
    CLI -->|"--mode review"| ReviewFlow

    ClassifyFlow & CheckFlow & ReviewFlow --> DocParser
    ClassifyFlow & CheckFlow & ReviewFlow --> PromptLoader
    ClassifyFlow & CheckFlow & ReviewFlow --> AOAIClient

    DocParser --> Input
    DocParser --> Mid

    PromptLoader --> Direction & Checklists

    AOAIClient <-->|"Chat Completions API\nHTTPS / Azure Private Endpoint"| AOAI

    ClassifyFlow -->|"書き込み"| Output
    CheckFlow -->|"書き込み"| Output2
    ReviewFlow -->|"書き込み"| Output
    ClassifyFlow & CheckFlow & ReviewFlow -->|"読み込み"| Mid
```
